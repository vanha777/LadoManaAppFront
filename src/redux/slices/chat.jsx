...
export function getContacts() {
  return async (dispatch) => {
    ...
    try {
      ...
      console.log(response.data.contacts) // this is contact lists featching
...
export function getCCData() {
  return async (dispatch) => {
    dispatch(slice.actions.startLoading());
    try {
      // this is contact lists featching
      const response = await axios.get('http://127.0.0.1:3333/chats',
        {
          withCredentials: true
        });
      console.log(response.data)
      dispatch(slice.actions.getCCData(response.data));
    } catch (error) {
      dispatch(slice.actions.hasError(error));
    }
  };
...
export function getConversations() {
  return async (dispatch) => {
    ...
    try {
      ...
      console.log(response.data.conversations) // this is conversatiions data API
...
export function getConversation(conversationKey) {
  return async (dispatch) => {
    dispatch(slice.actions.startLoading());
    try {
      const response = await axios.get('/api/chat/conversation', { // get conversation by id
...
export function markConversationAsRead(conversationId) {
  return async (dispatch) => {
    dispatch(slice.actions.startLoading());
    try {
      await axios.get('/api/chat/conversation/mark-as-seen', { // mark conversation as read
...
export function getParticipants(conversationKey) {
  return async (dispatch) => {
    dispatch(slice.actions.startLoading());
    try {
      const response = await axios.get('/api/chat/participants', { // get participants by conversation id
        params: { conversationKey },
      });
