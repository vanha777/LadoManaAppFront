...
export default function ChatNavItem({ conversation, openNav, isSelected, onSelect, ccData }) {
  console.log(ccData.to.profile.first_name)
  ...
  const isGroup = false //  details.otherParticipants.length > 1;

  const isUnread = true // conversation.unreadCount > 0;
  ...
  return (
    ...
    >
      <ListItemAvatar>
        ...
        <CustomAvatar
          key={ccData.to.to_id}
          alt={ccData.to.profile.first_name}
          src={ccData.to.profile.avatar_url}
          // BadgeProps={{
          //    badgeContent: <BadgeStatus status={details.otherParticipants[0].status} />,
          //  }}
          sx={{ width: 48, height: 48 }}
      ...
      {openNav && (
        <>
          <ListItemText
            primary={ccData.to.profile.first_name}
            primaryTypographyProps={{ noWrap: true, variant: 'subtitle2' }}
            secondary="testtestest"
            secondaryTypographyProps={{
              noWrap: true,
              variant: isUnread ? 'subtitle2' : 'body2',
              color: isUnread ? 'text.primary' : 'text.secondary',
          ...
          <Stack alignItems="flex-end" sx={{ ml: 2, height: 44 }}>
            ...
            >
              {formatDistanceToNowStrict(new Date(101010), {
                addSuffix: false,
              })}
            </Typography>
