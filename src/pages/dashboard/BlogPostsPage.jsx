...
export default function BlogPostsPage() {
  ...
  const getAllPosts = useCallback(async () => {
    try {
      const response = await axios.get('http://127.0.0.1:3333/posts', { withCredentials: true });
      ...
      <Container maxWidth={themeStretch ? false : 'lg'}>
        ...
        <Grid container spacing={3}>
          {(!posts.length ? [...Array(12)] : sortedPosts).map((post, index) =>
            post ? (
              <Grid key={post.post.id} item xs={12} sm={6} md={(index === 0 && 6) || 3}> { /* key can be just index */}
                <BlogPostCard posts={post} index={index} />
              </Grid>
            ) : (
